...
const App = () => {
  const { setUser, isAuthenticated, handleSetAuthenticated, user, setBlogs } =
    useContext(Context);
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const { data } = await axios.get(
          "https://blog-backend-two-chi.vercel.app/api/v1/user/myprofile",
          {
    ...
    const fetchBlogs = async () => {
      try {
        const { data } = await axios.get(
          "https://blog-backend-two-chi.vercel.app/api/v1/blog/all",
          { withCredentials: true }
        );
